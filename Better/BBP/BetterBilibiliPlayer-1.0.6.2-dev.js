...
// @version      1.0.6.2-dev
// @description  对B站播放页的一些界面的美化（含PGC内容支持，PGC禁用下载）
// @author       none
...
(function() {
    ...
    // 仅对 /video/ 或 /bangumi/ 生效
    if (!location.pathname.startsWith('/video/') && !location.pathname.startsWith('/bangumi/')) return;
    ...
    // ========== 全局变量 ==========
    let currentMode = 'watch';          // 'watch' 或 'download'
    ...
    function getPlayInfo() {
        ...
        if (w.__playinfo__) {
            // PGC 内容：数据在 result.video_info 下
            if (w.__playinfo__.result && w.__playinfo__.result.video_info && w.__playinfo__.result.video_info.dash) {
                return w.__playinfo__.result.video_info;
            }
            // UGC 内容：数据在 data 下
            if (w.__playinfo__.data && w.__playinfo__.data.dash) {
                return w.__playinfo__.data;
            }
            if (w.__playinfo__.dash) {
                return w.__playinfo__;
            }
        }
        if (w.__INITIAL_STATE__) {
            const state = w.__INITIAL_STATE__;
            if (state.videoData && state.videoData.dash) return state.videoData;
            if (state.playInfo && state.playInfo.dash) return state.playInfo;
            if (state.videoData?.data?.dash) return state.videoData.data;
        }
        if (w.__NEXT_DATA__?.props?.pageProps?.videoData?.dash) {
            return w.__NEXT_DATA__.props.pageProps.videoData;
        }
        return null;
    }
    ...
    function getVideoData() {
        ...
        if (playurlCache.has(currentId)) {
            const cached = playurlCache.get(currentId);
            if (cached && cached.dash && Array.isArray(cached.dash.video) && cached.dash.video.length > 0) {
                return cached;
            } else {
                playurlCache.delete(currentId);
    ...
    function getCid() {
        ...
        if (w.__playinfo__) {
            ...
            if (w.__playinfo__.result && w.__playinfo__.result.video_info && w.__playinfo__.result.video_info.cid) {
                return w.__playinfo__.result.video_info.cid;
            }
        }
    ...
    function getVideoUrl(qn, codec) {
        ...
        return matched ? (matched.baseUrl || matched.base_url || null) : null;
    }
    ...
    function getAudioUrl(audioId) {
        ...
        if (audio) return audio.baseUrl || audio.base_url || null;
        if (info.dash.flac) {
            if (info.dash.flac.audio && typeof info.dash.flac.audio === 'object') {
                ...
                return info.dash.flac.baseUrl || info.dash.flac.base_url || null;
            }
        }
    ...
    // 获取页面唯一标识（bvid 或 PGC 的 ep/ss）
    function getPageId() {
        const bvidMatch = location.pathname.match(/\/video\/([a-zA-Z0-9]+)/);
    ...
    function getDefaultAudioIdByQuality(qn) {
        if (qn <= 32) return 30216;
        if (qn === 64) return 30232;
        if (qn >= 80) return 30280;
        return null;
    }
    ...
    function renderListForStep(container, step, panel) {
        container.innerHTML = '';
        // 如果是 PGC 且处于下载模式，显示提示
        if (panel._isBangumi && currentMode === 'download') {
            const tip = document.createElement('div');
            tip.style.cssText = 'color:#aaa;font-size:14px;padding:20px 0;text-align:center;line-height:1.8;';
            tip.innerHTML = '由于技术原因，暂无法下载PGC内容<br><span style="font-size:12px;color:#666;">（仅支持网页效果及画质切换）</span>';
    ...
    function updatePanelForStep(panel) {
        ...
        // PGC 下载模式：显示提示，隐藏所有按钮
        if (panel._isBangumi && currentMode === 'download') {
            title.textContent = '下载不可用';
    ...
    function doShowPanel(btn) {
        ...
        panel._bvid = getBvid();
        panel._isBangumi = location.pathname.startsWith('/bangumi/');
    ...
    function showAboutDialog() {
        ...
        version.textContent = '当前版本 v1.0.6.2-dev';
    ...
    function observeRouteChanges() {
        ...
        let lastPageId = getPageId();

        const urlObserver = new MutationObserver(() => {
            if (location.href !== lastUrl) {
                lastUrl = location.href;
                if (location.pathname.startsWith('/video/') || location.pathname.startsWith('/bangumi/')) {
                    const newPageId = getPageId();
                    if (newPageId !== lastPageId) {
                        lastPageId = newPageId;
                        log('检测到视频切换（URL变化），执行完全重置并重新初始化', 'info');
                        addDebugLog('SPA 视频切换: ' + newPageId, true);
                        fullReset();
                    }
                }
            }
            ...
            const srcObserver = new MutationObserver(() => {
                ...
                if (currentSrc && currentSrc !== lastVideoSrc) {
                    ...
                    if (newPageId !== lastPageId) {
                        ...
                        addDebugLog('SPA 视频切换 (src): ' + newPageId, true);
                        fullReset();
                    }
                }
            });
    ...
    async function init() {
        ...
        if (playurlCache.has(targetPageId)) {
            log('缓存中存在当前视频数据，直接使用', 'info');
            dataAvailable = true;
            const cachedData = playurlCache.get(targetPageId);
