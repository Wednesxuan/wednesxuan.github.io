...
// @version      1.0.4.7-dev
// @description  美化B站播放器，修复下载（带进度条），非16:9黑边模糊填充，环境光等。使用缓存Map，彻底解决推荐视频画质无法获取问题。新增音视频合一MP4下载（最高1080P，但普遍为720P，无需FFmpeg）。修复cid获取，优先从__playinfo__读取，增加重试机制。MP4下载使用统一进度条。修复调试信息中码率显示为当前编码。
// @author       none
...
(function() {
    ...
    function getCid() {
        ...
        // 2. 从 window.__playinfo__ 提取（先顶层，再 data.cid）
        const w = typeof unsafeWindow !== 'undefined' ? unsafeWindow : window;
        if (w.__playinfo__) {
    ...
    function buildQualityListFromPlayurl(data) {
        ...
        videoList.forEach(item => {
            ...
            let desc = '未知';
            if (data.accept_description && data.accept_quality) {
                const idx = data.accept_quality.indexOf(qn);
                if (idx !== -1) desc = data.accept_description[idx];
            } else if (data.support_formats) {
                const fmt = data.support_formats.find(f => f.quality === qn);
                if (fmt) desc = fmt.new_description || fmt.display_desc || '未知';
            }
            if (desc === '未知' && width > 0 && height > 0) {
                const s = Math.min(width, height);
                if (s >= 1441) desc = '4K';
                else if (s >= 1081) desc = '2K';
                else if (s >= 721) desc = '1080P';
                else if (s >= 577) desc = '720P';
                else if (s >= 541) desc = '576P';
                else desc = s + 'P';
            }
    ...
    function getBvidFromInfo(info) {
        if (!info) return null;
        if (info.videoData && info.videoData.bvid) return info.videoData.bvid;
        if (info.bvid) return info.bvid;
        return null;
    }
    ...
    function applyQualityPreference() {
        ...
        if (!targetQn && qualityList.length > 0) targetQn = qualityList[0].id;
        if (!targetQn) return;
    ...
    function updateDebugInfo() {
        ...
        if (typeof player.getQuality === 'function') {
            try {
                ...
                if (q && q.nowQ !== undefined && !isNaN(q.nowQ)) {
                    qn = q.nowQ;
                    if (info && info.accept_description && info.accept_quality) {
                        const idx = info.accept_quality.indexOf(qn);
                        if (idx !== -1) desc = info.accept_description[idx];
                    }
                }
            } catch (_) {}
        }
        ...
        let actualVideo = null, actualAudio = null;
        const data = getVideoData();
        if (data?.dash?.video) {
            const qnVal = qn;
        ...
        if (data?.dash?.audio) {
            actualAudio = data.dash.audio.reduce((max, cur) => (cur.bandwidth > max.bandwidth ? cur : max), data.dash.audio[0]);
        }
        ...
        let aBitrate = '-';
        if (actualAudio?.bandwidth) {
            aBitrate = (actualAudio.bandwidth / 1000).toFixed(0) + ' kbps';
        }
    ...
    function renderListForStep(container, step, panel) {
        ...
        if (step === 1) {
            ...
            let codecForList = null;
            if (currentMode === 'download') {
                codecForList = selectedDownloadCodec;
            } else {
                codecForList = getActualCodecFromPlayer();
            }
            let qualityData = getFilteredQualityList(codecForList);
            if (qualityData.length === 0 && codecForList) {
                log('按编码过滤后无画质，尝试显示所有画质（可能编码名称不匹配）', 'warn');
    ...
    function renderSettingsContent(container, panel, titleElement) {
        ...
        ambientLabel.title = '该设置可能极度影响性能，如果对GPU非常自信可尝试开启。开启后视频内容将放大模糊作为网页背景，提供类似YouTube Ambient Mode的效果。';
    ...
    function showAboutDialog() {
        ...
        version.textContent = '当前版本 v1.0.4.7-dev';
    ...
    function onPlayurlDataReady(data) {
        ...
        if (!autoSwitchDone) {
            const formats = getFilteredQualityList();
            if (formats.length > 0) {
                applyQualityPreference();
            }
        }
    ...
    async function init() {
        ...
        if (playurlCache.has(targetBvid)) {
            ...
            // 2. 等待 __playinfo__ 或拦截器数据（最多 8 秒）
